...
from common import xlsx_file_name, local_schema_path
...
@dag(
    dag_id="process-remote-xlsx",
    schedule_interval="0 0 * * *",
    start_date=pendulum.datetime(2021, 1, 1, tz="UTC"),
    catchup=False,
    dagrun_timeout=datetime.timedelta(minutes=60),
)
def ProcessRemoteXlsx():
    @task
    def load(ti=None):
        ...
        obj = s3.Object(os.environ.get("AWS_BUCKET_NAME"), xlsx_file_name)
        file_content = obj.get()["Body"].read()
        session_id = uuid.uuid4()
        loader = Load(
            session_id=session_id, file_content=file_content, file_name=xlsx_file_name
        )
        loader.run()
