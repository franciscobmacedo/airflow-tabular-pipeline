...
from common import xlsx_local_file_path, local_schema_path
...
@dag(
    dag_id="process-local-xlsx",
    schedule_interval="0 0 * * *",
    start_date=pendulum.datetime(2021, 1, 1, tz="UTC"),
    catchup=False,
    dagrun_timeout=datetime.timedelta(minutes=60),
)
def ProcessLocalXlsx():
    @task
    def load(ti=None):
        session_id = uuid.uuid4()
        loader = Load(session_id=session_id, file_path=xlsx_local_file_path)
        loader.run()
